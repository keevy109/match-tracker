...
import {sanitizeNews,safeImage,newsDate,plainText} from './news-content.js';
let items=[],current=null,image='',imageData='',dirty=false,saving=false,imageToken=0,imageBusy=false;
const $=id=>document.getElementById(id);
function markDirty(){dirty=true;try{localStorage.setItem('ssv_news_draft_'+current,JSON.stringify(read()));}catch{}}
...
function open(item){
  ...
  imageToken++;imageBusy=false;current=item?.id||crypto.randomUUID();
  let data=item;
...
function read(){return {...items.find(n=>n.id===current),id:current,title:$('newsTitle').value.trim(),date:$('newsDate').value,excerpt:$('newsExcerpt').value.trim(),published:$('newsPublished').checked,body:sanitizeNews($('newsBody').innerHTML),image:imageData||image};}
...
async function save(){
 if(saving)return;
 if(imageBusy){$('newsSaveStatus').textContent='Das Bild wird noch vorbereitet.';return;}
 ...
 try{
  localStorage.setItem('ssv_news_draft_'+current,JSON.stringify(data));
  ...
  const next=items.filter(n=>n.id!==current).concat(data);await api.save('news',next);items=next;image=data.image;imageData='';dirty=false;localStorage.removeItem('ssv_news_draft_'+current);render();$('newsSaveStatus').textContent='Lokal gespeichert. Mit dem nächsten GitHub-Push wird der Beitrag veröffentlicht.';
 }catch(error){$('newsSaveStatus').textContent=error.message+' Dein Entwurf bleibt in diesem Browser erhalten.';}finally{saving=false;lockEditor(false);}
}
export async function init(){
 ...
 $('newsShowPreview').onclick=()=>{const data=read();$('newsPreview').hidden=false;$('newsPreviewTitle').textContent=data.title;$('newsPreviewDate').textContent=newsDate(data.date);$('newsPreviewBody').innerHTML=data.body;$('newsPreviewImage').src=safeImage(data.image);$('newsPreviewImage').hidden=!data.image;};
