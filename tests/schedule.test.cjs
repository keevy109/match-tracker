...
const c=vm.createContext({URLSearchParams,window:{location:{search:''}},Date,teams:[{id:1,name:'Unser FC',isOurTeam:true},{id:2,name:'Gegner FC'}],getOurTeamName:()=> 'Unser FC',getCanonicalTeamName:name=>name,schedule:[],document:{getElementById:id=>fields[id]},showToast(){},renderSpielplan(){},save(){saves++;}});vm.runInContext(source,c);return {c,fields,saves:()=>saves};}
